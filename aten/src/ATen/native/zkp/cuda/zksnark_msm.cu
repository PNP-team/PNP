...
static void mult_pippenger_inf(Tensor& self, const Tensor& points, const Tensor& scalars, Tensor& workspace, int64_t smcount, int64_t blob_u64)
{
    AT_DISPATCH_FQ_MONT_TYPES(points.scalar_type(), "msm_cuda", [&] {
        ...
        auto bucket_ptr = reinterpret_cast<bucket_h*>(workspace.mutable_data_ptr<uint64_t>());
        auto temp_ptr = reinterpret_cast<uint8_t*>(workspace.mutable_data_ptr<uint64_t>() + blob_u64);
        ...
        mult_pippenger<point_t>(self_ptr, point_ptr, npoints, scalar_ptr, bucket_ptr, temp_ptr, smcount, false, ffi_affine_sz); 
    });
}
...
Tensor msm_zkp_cuda(const Tensor& points, const Tensor& scalars, 
    ...
    size_t d_blob_sz = d_buckets_sz * sizeof(uint64_t) * num_uint64(points.scalar_type())*4
    ...
    auto workspace = at::empty({blob_u64 + digit_u64},
      ScalarType::ULong, points.options().layout(), points.options().device(), points.options().pinned_memory(), c10::nullopt);

    auto out = at::empty({(nwins * MSM_NTHREADS/1 * 2 + smcount * BATCH_ADD_BLOCK_SIZE / WARP_SZ) * 4,num_uint64(points.scalar_type())},
      points.options());
