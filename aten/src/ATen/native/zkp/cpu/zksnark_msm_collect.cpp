...
static void pippenger_collect(Tensor& self, const Tensor& step1res, int64_t npoints){
    AT_DISPATCH_FQ_MONT_TYPES(self.scalar_type(), "msm_cpu", [&] {
        ...
        auto lenofone = step1res.numel()/ (num_uint64(step1res.scalar_type())*4) - lenofres;
        
        auto self_ptr = reinterpret_cast<point_t*>(self.mutable_data_ptr<scalar_t>());
        auto res_ptr = reinterpret_cast<bucket_t*>(step1res.mutable_data_ptr<scalar_t>());
        auto ones_ptr = reinterpret_cast<bucket_t*>(step1res.mutable_data_ptr<scalar_t>()) + lenofres;
        collect_t<bucket_t, point_t, affine_t, scalar_t::compute_type::coeff_t> msm_collect{npoints};
