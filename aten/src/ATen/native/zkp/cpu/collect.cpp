...
public:
    void collect(point_t* out, const bucket_t* res, const bucket_t* ones, uint32_t lenofone)
    {
        std::cout<<(res)->X<<std::endl;
        std::cout<<(ones)->Y<<std::endl;
        struct tile_t {
            uint32_t x, y, dy;
            point_t p;
            tile_t() {}
        ...
        bucket_t bucket_one = sum_up(ones, lenofone);
        point_t one = bucket_one;
        one.inf();
...
public:
    point_t integrate_row(const bucket_t* row, uint32_t lsbits)
    {
        ...
        if (lsbits-1 <= NTHRBITS) {
            ...
            bucket_t res, acc = *(row + i * MSM_NTHREADS/bucket_t::degree + 1);
            if (mask)   res.inf();
            else        res = acc;

            while (i--) {
                acc.add(*(row + i * MSM_NTHREADS/bucket_t::degree + 1));
                if ((i & mask) == 0)
                    res.add(acc);
            }
        ...
        point_t  res = *(row + i * MSM_NTHREADS/bucket_t::degree); 
        bucket_t acc = *(row + i * MSM_NTHREADS/bucket_t::degree + 1);
        ...
        while (i--) {
            ...
            for (size_t j = 0; j < lsbits-1-NTHRBITS; j++)
                raise.dbl();
            res.add(raise);
            point_t row_i0 = *(row + i * MSM_NTHREADS/bucket_t::degree);
            res.add(row_i0);
            if (i)
            {
                point_t row_i1 = *(row + i * MSM_NTHREADS/bucket_t::degree + 1);
                acc.add(row_i1);
            }
