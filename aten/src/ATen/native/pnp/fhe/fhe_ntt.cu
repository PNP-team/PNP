...
#include <stddef.h>
#include <stdint.h>
#include "fhe_ntt/ntt_kernel/ntt.cuh"
#include "fhe_ntt/parameters/parameters.cuh"
...
static void params_ntt_template(
    Tensor& self,
    Tensor& local_params,
    bool is_intt) {
...
Tensor params_ntt_cuda(
  ...
  auto params = at::empty(
      ...
      pin_memory,
      c10::nullopt);
  params_ntt_template(params, local_params, is_intt);
  return params;
}
...
Tensor ntt_fhe_cuda(
  ...
  ntt_fhe(output, params, is_intt, is_coset);
  return output;
}

Tensor& ntt_fhe_cuda_(
    Tensor& inout,
    const Tensor& params,
    bool is_intt,
    bool is_coset) {
  ntt_fhe(inout, params, is_intt, is_coset);
  return inout;
}

Tensor& ntt_fhe_out_cuda(
    const Tensor& inout,
    const Tensor& params,
    bool is_intt,
    bool is_coset,
